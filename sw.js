...
const CACHE = `${PREFIX}workspace-v2`;
const APP_SHELL = [
  "/index.html", "/manifest.webmanifest", "/css/tokens.css", "/css/app.css", "/css/auth.css",
...
self.addEventListener("fetch", event => {
  ...
  // Never store API responses, authenticated documents or one-time links.
  if (url.pathname.startsWith("/v1/") || url.pathname === "/health" || url.pathname === "/ready") return;
  if (event.request.mode === "navigate") {
    if (url.pathname !== "/" && url.pathname !== "/index.html") return;
    event.respondWith(fetch(event.request).catch(() => caches.match("/index.html").then(hit => hit || new Response("Workspace est hors ligne.", { status: 503, headers: { "Content-Type": "text/plain; charset=utf-8" } }))));
    return;
  }
  const allowed = APP_SHELL.includes(url.pathname) || /^\/assets\/icons\/Iconly[A-Za-z0-9]+\.svg$/.test(url.pathname);
  if (!allowed) return;
  event.respondWith(fetch(event.request).then(response => {
    ...
    return response;
  }).catch(async () => (await caches.match(url.pathname)) || new Response("Ressource hors ligne", { status: 503 })));
});
