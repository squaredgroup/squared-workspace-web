...
function stopRealtime(){
  clearTimeout(realtimeReconnectTimer);clearTimeout(realtimeRefreshTimer);
  const old=state.realtime;state.realtime=null;
  try{old?.close()}catch{/* already closed */}
}
function connectRealtime(){
  if(!uiReady||!state.accessToken||!state.online)return;
  stopRealtime();
  const wsURL=realtimeURL();wsURL.searchParams.set("accessToken",state.accessToken);
  try{
    const socket=new WebSocket(wsURL);state.realtime=socket;
    socket.addEventListener("open",()=>{reconnectDelay=1000});
    socket.addEventListener("message",event=>{
      if(socket!==state.realtime)return;
      let message;try{message=JSON.parse(event.data)}catch{return}
      if(message.type==="connected")return;
      clearTimeout(realtimeRefreshTimer);
      realtimeRefreshTimer=setTimeout(async()=>{
        try{await loadWorkspace();if(["dashboard","today","messages","notifications","activity"].includes(state.route.section))renderCurrent()}catch{/* HTTP error remains visible on the next action */}
      },500);
    });
    socket.addEventListener("close",()=>{
      if(socket!==state.realtime)return;
      state.realtime=null;
      if(uiReady&&state.accessToken&&state.online){
        realtimeReconnectTimer=setTimeout(connectRealtime,reconnectDelay);
        reconnectDelay=Math.min(30000,reconnectDelay*2);
      }
    });
  }catch{/* HTTP remains usable when WebSocket is unavailable */}
}
async function authenticated(){
  await Promise.all([loadMe(),loadWorkspace(),loadDomainCatalog().catch(()=>null)]);
  if(!state.user||!state.accessToken)return;
  uiReady=true;buildShell();uiSignature=signature();connectRealtime();
}
function resetInterface(reason=""){
  uiReady=false;uiSignature="";renderGeneration++;refs={};stopRealtime();
  document.querySelector("#portal-root")?.replaceChildren();
  document.querySelectorAll(".overlay").forEach(node=>node.remove());
  document.querySelector(".toast-stack")?.replaceChildren();
  renderAuth(authenticated,{message:reason==="expired"?"Votre session a expiré. Reconnectez-vous.":""});
}
window.addEventListener("sq:session-ended",event=>resetInterface(event.detail?.reason));
window.addEventListener("offline",stopRealtime);
window.addEventListener("online",()=>{if(uiReady)connectRealtime()});
async function bootstrap(){
  if("serviceWorker" in navigator)navigator.serviceWorker.register("/sw.js",{updateViaCache:"none"}).then(reg=>reg.update()).catch(()=>{});
  const restoring=!authContext.action&&hasStoredSession();
  renderAuth(authenticated,{restoring});
  if(!restoring)return;
  try{await refreshSession();await authenticated()}
  catch(error){
    if(error.name!=="AbortError")renderAuth(authenticated,{message:errorMessage(error)});
  }
}
