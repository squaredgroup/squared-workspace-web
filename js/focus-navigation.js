...
export function focusTabs(){
  ...
  return h("nav",{class:"mobile-tabs focus-tabs","aria-label":"Navigation mobile"},...entries.map(item=>{
    const active=item===matched||(!matched&&item.key==="spaces");
    return h("button",{type:"button",class:active?"active":"","aria-label":item.title,"aria-current":active?"page":null,onClick:()=>go(item.key)},icon(item.key==="spaces"?"grid":item.key,22),h("span",{text:item.title}));
  }));
}
export function focusHeader({search,tools,refresh}){
  const route=state.route.section;
  const title=route==="dashboard"?"Accueil":route==="notifications"?"À traiter":SECTIONS[route]?.title||"Workspace";
  const head=h("header",{class:"topbar focus-topbar"},h("div",{class:"breadcrumbs"},h("span",{class:"breadcrumb-line",text:state.workspace?.workspace?.name||"Squared Workspace"}),h("div",{class:"top-title",text:title})),h("div",{class:"top-actions"},iconButton("search","Recherche",search)));
