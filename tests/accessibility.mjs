...
const payloadFor=path=>path==="/v1/auth/password"?session:path==="/v1/me"?user:path==="/v1/workspace"?workspace:path==="/v1/domain-data/catalog"?{kinds:[]}:{};
...
try{
  ...
  if(auth.violations.length)throw new Error(`Auth: ${auth.violations.map(value=>`${value.id} (${value.nodes.length})`).join(", ")}`);
  ...
  if(dashboard.violations.length)throw new Error(`Dashboard: ${dashboard.violations.map(value=>`${value.id} (${value.nodes.length})`).join(", ")}`);
