...
with sync_playwright() as playwright:
    ...
    page.get_by_role("button", name="Se connecter", exact=True).click()
    expect(page.get_by_role("heading", name="Tableau de bord", exact=True)).to_be_visible()
    mobile_emoji = page.get_by_role("img", name="Main qui salue", exact=True)
    expect(mobile_emoji).to_be_visible()
    expect(page.locator(".greeting-emoji")).to_have_text("👋")
    assert mobile_emoji.evaluate("emoji=>{const box=emoji.getBoundingClientRect();return box.width>=30&&box.height>=30&&box.left>=0&&box.right<=innerWidth}")
    for _ in range(2):
        page.reload()
        expect(page.get_by_role("heading", name="Tableau de bord", exact=True)).to_be_visible()
        mobile_emoji = page.get_by_role("img", name="Main qui salue", exact=True)
